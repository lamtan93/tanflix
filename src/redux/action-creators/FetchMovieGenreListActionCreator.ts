...
const fetchMovieGenreList = () => async (dispatch: Dispatch<MovieGenreListAction>) => {
  dispatch({
    type: MovieGenreListActionType.FETCH_MOVIE_GENRE_LIST_REQUEST,
  });
  try {
    const rawDataFromAPI = await
    sendAPIRequest<IRawMovieGenreListDataFromAPI>('genre/movie/list', 'GET')
      .then((data) => data);
